"""Example multiscale datasets for progressive loading integrations.
...
directly to a renderer integration or wrapped in an ``ArrayPyramidSource``.
...
from ..sources import OMEZarrSource
...
def _escape_to_rgb(chunk, maxiter, dtype: Any = np.uint8):
            ...
            for yi in range(n_chunks):
                for xi in range(n_chunks):
                    ...
                    if done % max(1, total // 10) == 0 or done == total:
                        LOGGER.info("Level %d: %d/%d chunks", level, done, total)
...
def local_zarr_dataset(
    ...
    dict
        ...
        a progressive renderer integration.
    """
    path = Path(path)
    if not path.exists():
    ...
    multiscales = cast(list[dict[str, Any]], group.attrs.get("multiscales", [{}]))
    level_paths = [d["path"] for d in multiscales[0].get("datasets", [])]
    if not level_paths:
        level_paths = sorted(k for k in group if k.isdigit())

    arrays = [cast(zarr.Array, group[p]) for p in level_paths]
    ndim = arrays[0].ndim
    scale_factors = [
