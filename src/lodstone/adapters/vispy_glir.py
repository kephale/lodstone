...
The patch is opt-in: renderer integrations call :func:`install` when they
begin progressive streaming and may call :func:`uninstall` during teardown.

Run ``python -m lodstone.adapters.vispy_glir`` for a standalone
...
def remove_drain_callback(method) -> None:
    _drain_callbacks[:] = [
        ref for ref in _drain_callbacks if (cb := ref()) is not None and cb != method
    ]
...
def _metered_parse(parser, commands, state, force_defer=False):
    ...
    for id_ in [id_ for id_, val in parser._objects.items() if val == JUST_DELETED]:
...
def install(
    ...
    if _original_flush is None:
        ...
        LOGGER.info(
            "GLIR texture upload metering installed (budget=%d B/frame, slab=%d B)",
            frame_budget_bytes,
            slab_bytes,
        )
    return True
...
def _benchmark():  # pragma: no cover - manual profiling tool
    ...
    volume = scene.visuals.Volume(base)
    volume.parent = view.scene
    view.camera = scene.cameras.TurntableCamera(fov=60.0)
