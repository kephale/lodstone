...
def test_getitem_composes_display_crop_and_point_selection(dask_array, base_array):
    """Clients may crop displayed dims before indexing point dims."""
    vdata = VirtualData(dask_array)
...
def test_set_chunk_updates_pixels_and_provenance_under_one_lock(dask_array, base_array):
...
def test_concurrent_reads_and_writes(dask_array, base_array):
    ...
    threads = [threading.Thread(target=t) for t in (writer, mover, reader, reader)]
    for t in threads:
        t.start()
    for t in threads:
        t.join()
