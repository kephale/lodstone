...
from lodstone.adapters import vispy_glir as gm
...
def test_single_slice_too_large_splits_rows(parser):
    ...
    assert [c[2] for c in parser.executed] == [(3, 256 * i, 0) for i in range(4)]
...
def test_redundant_gl_state_calls_skipped(parser):
    ...
    leftover = gm._metered_parse(parser, [("FUNC", "glBlendFunc", "one", "one")], state)
    assert leftover == []
...
def test_deletes_deferred_to_quiet_flush(monkeypatch):
    ...
    parser.add_texture3d(1)
    try:
        assert gm.install(frame_budget_bytes=512 * 2**10, slab_bytes=128 * 2**10)
        queue = glir.GlirQueue()
        data = np.zeros((16, 256, 256), dtype=np.uint8)  # 1 MiB
        queue.command("DATA", 1, (0, 0, 0), data)
...
def test_delete_drain_is_paced(monkeypatch):
    ...
    try:
        ...
        while state.carry:
            state.reset_budget()
            queue.flush(parser)
        executed_deletes = lambda: sum(c[0] == "DELETE" for c in parser.executed)
        before = executed_deletes()
        state.reset_budget()
        queue.flush(parser)  # quiet: paced batch only
        assert executed_deletes() - before == gm.DELETE_DRAIN_PER_FLUSH
        assert len(state.deferred_deletes) == (n_deletes - gm.DELETE_DRAIN_PER_FLUSH)
    ...
    try:
        ...
        for _ in range(64):
            ...
            assert flushed <= budget + slab, (
                f"one flush uploaded {flushed} bytes (> budget {budget} + slab {slab})"
            )
            total += flushed
            if not state.carry:
                break
